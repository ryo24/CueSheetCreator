...
3. 各シーンに進行内容、照明・きっかけ、備考、必要に応じて画像を登録します。
...
### `v1.1.0` の主な修正（2026-04-06）
- 印刷プレビューと PDF 出力の向きを A4 縦向きに修正
- 画像プレビューを 16:9 ベースで扱いやすい表示に調整
- 表ヘッダーと各列の位置ずれを修正
- `Ctrl+S` / `Command+S` で `.cuesheet` を直接書き出せるように変更
- 列名の表記を調整し、`照明・きっかけ (LIGHTING / CUE)` に統一
