...
We break the "template" look by treating the CueSheet not as a spreadsheet, but as a live editorial document. We utilize **intentional asymmetry**, where sidebars and content areas have varying weights, and **High-Contrast Typography Scales** that juxtapose an expansive Serif display with a functional, condensed Sans-Serif UI. The goal is to make the user feel they are "curating" an experience rather than just entering data.
...
- **Surface Tiers**: We use `surface-container-lowest` (`#ffffff`) for the actual CueSheet "cells" and `surface` (`#f8f9fa`) for the workspace background.
...
- **UI & Labels (Manrope):** A modern, geometric sans-serif used for the "data" of the CueSheet. 
...
### Structured Tables (The CueSheet)
...
- **Don't** crowd the CueSheet. If a table feels tight, increase the row padding to `spacing-4` (1rem).
